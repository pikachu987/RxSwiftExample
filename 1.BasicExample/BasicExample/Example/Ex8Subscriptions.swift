...
//        self.share()
    ...
    func multicast() {
        let timer = Observable<Int>.interval(RxTimeInterval.seconds(1), scheduler: MainScheduler.instance).take(4)
        ...
        multicast.delaySubscription(RxTimeInterval.seconds(2), scheduler: MainScheduler.instance)
            .subscribe { event in
    ...
    func publish() {
        let publish = Observable<Int>.interval(RxTimeInterval.seconds(1), scheduler: MainScheduler.instance)
            .take(4)
            .publish()
        _ = publish.connect()
        publish.subscribe { print("first: \($0)") }
            .disposed(by: self.disposeBag)
        publish.delaySubscription(RxTimeInterval.seconds(1), scheduler: MainScheduler.instance)
    ...
    func replay() {
        let timer = Observable<Int>.interval(RxTimeInterval.seconds(1), scheduler: MainScheduler.instance).take(5)
        let replay = timer.replay(2)
        _ = replay.connect()
        replay.subscribe { print("first: \($0)") }
            .disposed(by: self.disposeBag)
        replay.delaySubscription(RxTimeInterval.seconds(4), scheduler: MainScheduler.instance)
            .subscribe { print("second: \($0)") }
            .disposed(by: self.disposeBag)
    }
    ...
    func share() {
        ...
        share.subscribe { print("first: \($0)") }
            .disposed(by: self.disposeBag)
        share.delaySubscription(RxTimeInterval.seconds(4), scheduler: MainScheduler.instance)
            .subscribe { print("second: \($0)") }
            .disposed(by: self.disposeBag)
    }
}
